...
| Araç kataloğu (JSON seed) | `OtoRehber/Data/catalog/*.json` + `OtoRehber.Infrastructure/Data/CatalogSeed/CatalogSeeder.cs` (bkz. `Data/catalog/README.md`) |
...
- [x] Marka + segment landing sayfaları — `CatalogController` `/marka/{slug}` + `/segment/{slug}`
  (`Slugify` Türkçe karakter sadeleştirme; bilinmeyen slug → 404; sitemap'e eklendi;
  araç detayında marka/segment link rozetleri). Ortak `_CarCard` partial'ı çıkarıldı (Home + Catalog).
- [x] Footer'da marka/segment listesi — `CatalogMenuViewComponent` (10 dk cache; admin CRUD'da invalidate)
- [x] Hakkımızda, İletişim (Faz 1'de eklendi)
...
| 2026-08-29 | katalog | **JSON araç kataloğu seed sistemi** — `OtoRehber/Data/catalog/*.json` + `CatalogSeeder`; startup'ta çalışır (yerel+prod). İlk batch: `00-amiral-batch.json` = 25 varyant | (F commit) |
| 2026-08-29 | katalog | Seeder **bildirimsel senkrona** çevrildi: `Car.Source` kolonu (migration `AddCarSource`), ekle/benimse/**buda** (JSON = doğruluk kaynağı); yorumlu araç budanmaz. `Catalog:Sync` bayrağı (testlerde kapalı). Kesin granülerlik kuralları (`Data/catalog/README.md`): her plaka/nesil/makyaj/motor/şanzıman ayrı kayıt, `ModelName = Plaka (kasa kodu, yıl-yıl)` | (G commit) |
...
```
- `__` = appsettings'teki `:`.
- **Admin şifresini unuttuysan / `AdminSeed__Password` değiştirdiğin hâlde giriş yapamıyorsan:**
  seed mevcut kullanıcının şifresini GÜNCELLEMEZ (sadece yoksa oluşturur). Çözüm:
  Railway'de geçici olarak `AdminSeed__ResetPassword=true` ekle → redeploy → artık
  `AdminSeed__Password` ile giriş yapabilirsin (e-posta da onaylanır, kilit açılır) →
  **sonra `AdminSeed__ResetPassword` değişkenini KALDIR.**
