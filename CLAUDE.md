...
| 2026-08-29 | katalog | **JSON araç kataloğu seed sistemi** — `OtoRehber/Data/catalog/*.json` + `CatalogSeeder` (idempotent, anahtar `Brand+Model+Engine`, `Catalog__ForceUpdate` ile yeniden yazma); startup'ta çalışır (yerel+prod). İlk batch: `00-amiral-batch.json` = en çok aranan 25 varyant, motor+şanzıman ayrı kayıt, tam detaylı uzman özeti/forum izlenimi/kronik arıza/artı-eksi/km barajı | (F commit) |
